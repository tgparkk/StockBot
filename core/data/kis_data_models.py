...
class MomentumData:
    ...
    def from_kis_data(cls, current: KISCurrentPrice, historical: List[KISHistoricalData],
        ...
        # 이동평균 계산
        closes = [h.stck_clpr for h in historical]
        ma_5 = sum(closes[-5:]) / 5 if len(closes) >= 5 else closes[-1]
        ...
        # 수익률 계산
        return_1d = ((current.stck_prpr - closes[-1]) / closes[-1]) * 100 if closes else 0
        return_5d = ((current.stck_prpr - closes[-5]) / closes[-5]) * 100 if len(closes) >= 5 else 0
