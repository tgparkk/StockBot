...
class WorkerManager:
    ...
    def start_all_workers(self, bot_instance):
        """모든 워커 시작"""
        try:
            # 1. 포지션 모니터링 워커
            ...
            # 3. 🆕 웹소켓 연결 모니터링 워커
            if self._should_monitor_websocket(bot_instance):
    ...
    def start_position_monitor(self, bot_instance):
        """포지션 모니터링 워커 시작"""
        try:
    ...
    def start_auto_sell_worker(self, bot_instance):
        """자동 매도 워커 시작"""
        try:
            if self.shutdown_event.is_set():
                return
    ...
    def _should_monitor_websocket(self, bot_instance) -> bool:
        """웹소켓 모니터링 필요 여부 확인"""
        try:
            # 웹소켓 관리자가 있고, 웹소켓 우선 사용 설정인 경우
            return (hasattr(bot_instance, 'websocket_manager') and 
                    bot_instance.websocket_manager is not None and
                    hasattr(bot_instance, '_should_prefer_websocket') and
                    bot_instance._should_prefer_websocket())
        except:
            return False

    def start_websocket_monitor(self, bot_instance):
        """🎯 웹소켓 연결 모니터링 워커 시작"""
        try:
            if self.shutdown_event.is_set():
                return
    ...
    def _websocket_monitor_worker(self, bot_instance):
