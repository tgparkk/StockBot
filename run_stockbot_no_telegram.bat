@echo off
title StockBot (텔레그램 봇 비활성화)
chcp 65001 > nul

echo.
echo ========================================
echo 🤖 StockBot (텔레그램 봇 비활성화 모드)
echo ========================================
echo.
...
:: 환경설정 파일 확인 (텔레그램 설정은 건너뜀)
if not exist "config\.env" (
    echo.
    echo ⚠️  환경설정 파일이 없습니다!
    echo    config\.env 파일을 생성하고 KIS API 키를 설정하세요.
    echo.
    echo    필수 설정:
...
echo.
echo 📋 의존성 확인 중...
python -c "import pandas, numpy, peewee" 2>nul
if errorlevel 1 (
...
echo.
echo 🤖 텔레그램 봇이 비활성화된 상태로 실행됩니다.
echo    원격 제어는 불가능하지만 거래 시스템은 정상 작동합니다.
echo.
echo 🚀 StockBot 시작 중...
echo.
